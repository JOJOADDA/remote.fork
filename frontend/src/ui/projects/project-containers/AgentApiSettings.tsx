...
export function AgentApiSettings({ secrets, onSave }: { secrets: ProjectSecret[]; onSave: (key: string, value: string) => Promise<void> }) {
  ...
  return (
    <section class="rounded-md border border-accent-blue/20 bg-accent-blue/[0.04] p-3 space-y-3">
      ...
      <ProviderCard title="Azure OpenAI / Foundry" description="Codex requires an Azure deployment compatible with the Responses API and tool calling. Use a GPT Codex deployment below; DeepSeek chat-only deployments are not compatible with Codex's agent loop." fields={FIELDS.filter((field) => field.group === "azure")} draft={draft} onChange={(key, value) => setDraft((current) => ({ ...current, [key]: value }))} onSave={() => void saveGroup("azure")} saving={saving === "azure"} saved={saved === "azure"} />
...
const AZURE_MODEL_OPTIONS = ["gpt-5.6-terra", "gpt-5.3-codex", "gpt-5.2-codex", "gpt-5.1-codex", "gpt-5-codex"];
...
function ProviderCard({ title, description, fields, draft, onChange, onSave, saving, saved }: { title: string; description: string; fields: ReadonlyArray<Field>; draft: Record<string, string>; onChange: (key: string, value: string) => void; onSave: () => void; saving: boolean; saved: boolean }) {
  return (
    <div class="rounded-md border border-white/10 bg-white/[0.03] p-3 space-y-2.5">
      ...
      <div class="grid gap-2 md:grid-cols-2">{fields.map((field) => <label key={field.key} class="space-y-1"><span class="block text-[11.5px] text-ink-300 font-mono">{field.key}</span>{field.key === "AZURE_OPENAI_DEPLOYMENT" ? <><select value={AZURE_MODEL_OPTIONS.includes(draft[field.key] ?? "") ? draft[field.key] : ""} onChange={(event) => onChange(field.key, (event.target as HTMLSelectElement).value)} class="w-full h-9 px-2.5 rounded border border-white/10 bg-black/30 text-[12.5px] text-ink-50 focus:outline-none focus:border-accent-blue/50"><option value="">Choose a Codex deployment / Auto</option>{AZURE_MODEL_OPTIONS.map((model) => <option key={model} value={model}>{model}</option>)}</select><input type="text" value={draft[field.key] ?? ""} onInput={(event) => onChange(field.key, (event.target as HTMLInputElement).value)} placeholder={field.placeholder} autoComplete="off" spellcheck={false} class="w-full h-9 px-2.5 rounded border border-white/10 bg-black/30 text-[12.5px] font-mono text-ink-50 placeholder-ink-500 focus:outline-none focus:border-accent-blue/50" /></> : <input type="password" value={draft[field.key] ?? ""} onInput={(event) => onChange(field.key, (event.target as HTMLInputElement).value)} placeholder={field.placeholder} autoComplete="off" spellcheck={false} class="w-full h-9 px-2.5 rounded border border-white/10 bg-black/30 text-[12.5px] font-mono text-ink-50 placeholder-ink-500 focus:outline-none focus:border-accent-blue/50" />}</label>)}</div>
